...
def readms2csvpdreg():
    df = pd.read_csv('the MS2 summary from44218.csv', sep='\t') #'debug1.csv the MS2 summary from44218.csv'
    print(df.dtypes)
    hitspectrumlist = []
    err = 0
    notfound = 0
    ...
    with open('ms2filteredtotallistwithH_NGbrain20221129_50ppm.csv', 'w', newline='') as zz:
        roww = csv.writer(zz)
