...
#changelog: 20241011 v0.8: general test finished. Save a new file for organizing code.
#changelog: 20241012 v0.8a: prepared and organized the file for further improvement.

#considering if I should merge this file to annotationhelper.py
...
#    if type == "PerMe":
#     monosaccharide_masses = sugarinfo.permethylated_masses
...
'H2O': 18.0151,
'reduced': 62.0778,
'nonreduced': 46.0465 #not sure if it's 46.0465 or 46.0783
}
...
def calc_protonated_mass(row,type,debug = False):
    ...
    total_mass += 1.007325 + etc_masses.get(type)#('reduced')#get(type)
...
def adding_protonated_and_observed_mass(df): #type should be solved here
    #df['theo protonated mass'] = df.apply(calc_protonated_mass, axis = 1)
    df['theo protonated mass'] = df.apply(lambda row: calc_protonated_mass(row, type=type), axis=1) #GPT suggestion to use lambda
...
createnormailzedionlistcsv(ionlist, preprocessed_df,ion_df, R"G:\其他電腦\My Computer\GlycoMSParser\2024review\zfNGintestinefixed20241012.csv")
...
add_unique_id(R"G:\其他電腦\My Computer\GlycoMSParser\2024review\zfNGintestinefixed20241012.csv", extname="intestine")