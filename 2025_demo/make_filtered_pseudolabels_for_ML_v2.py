...
def normalize_structure_column(df: pd.DataFrame, struct_col: str="Structure") -> pd.Series:
    out = []
    for v in df[struct_col].fillna("").tolist():
    ...
    return pd.Series(out, index=df.index, name="Structure")

def pick_score_column(df: pd.DataFrame) -> Optional[str]:
    for cand in ("score", "pseudolabel_score", "pl_score", "Score", "ion score"):
...
def salvage_labels_with_manual(df: pd.DataFrame, manual_csv: str) -> pd.DataFrame:
    try:
        ...
        return df
    scan_col = None
    for cand in ("MS2scan_no","ms2scan_no","ScanNo","scan","scan_no"):
        if cand in mdf.columns:
            scan_col = cand
            break
    if scan_col is None:
        print("[salvage] Manual CSV lacks MS2scan_no; skip salvage.")
        return df
    if "Structure" not in mdf.columns:
        print("[salvage] Manual CSV lacks Structure; skip salvage.")
        return df
    mdf["Structure"] = normalize_structure_column(mdf, "Structure")
    ms2_to_struct = mdf.set_index(scan_col)["Structure"].to_dict()
    def _maybe_override(row):
        sc = row.get("MS2scan_no", None)
        if pd.isna(sc):
        ...
        try:
            ...
            key = sc
        return ms2_to_struct.get(key, row["Structure"])
    df["Structure"] = df.apply(_maybe_override, axis=1)
    return df
...
def make_trainable(
    ...
    for col in ["MS2scan_no", "Structure"]:
        if col not in df.columns:
            raise ValueError(f"Input missing required column: {col}")
    df["Structure"] = normalize_structure_column(df, "Structure")
    if score_threshold is not None:
        sc = None
        for c in ("score","pseudolabel_score","pl_score","Score"):
            if c in df.columns:
                sc = c; break
        if sc is None:
    ...
    if salvage_manual_csv:
        df = salvage_labels_with_manual(df, salvage_manual_csv)
    keep_cols = []
    if "protonatedmass" in df.columns:
        keep_cols.append("protonatedmass")
    keep_cols.append("MS2scan_no")
...
def main():
    ap = argparse.ArgumentParser(description="Convert pseudolabeled TSV to trainable CSVs with ion sheet, thresholds, and salvage.")
    ap.add_argument("--pseudotab", required=True)
    ap.add_argument("--out-prefix", required=True)
    ap.add_argument("--manual-csv", default=None, help="Manual MSlist or full CSV including MS2scan_no and Structure.")
