...
def open_prepare_dataset_window():
    ...
    def on_tree_select(event):
        ...
        # Check if it's a valid sample in the experiment
    ...
    def link_and_validate_sample(exp_name, sample_name):
        sample_name = clean_sample_name(sample_name)  # ✅ normalize name
        sample = experiment_projects[exp_name]["samples"][sample_name]
        ...
        # Check CSV format
        try:
            messagebox.showinfo("validating csv placeholder")
            #import mspval
            #mspval.validatecsv(sample["csv"])  # You handle exceptions/logs inside
        except Exception as e:
            messagebox.showerror("CSV Validation Failed", str(e))
            return

        # Check Excel format
        try:
            messagebox.showinfo("validating csv placeholder")
            #mspval.validateanno(sample["excel"])  # You handle exceptions/logs inside
        except Exception as e:
        ...
        # TODO: Save or update .method file based on sample info
        messagebox.showinfo("Success", f"Sample '{sample_name}' under '{exp_name}' validated and linked successfully.")
    ...
    tk.Button(button_frame, text="Load Method", command=load_method_file).grid(row=2, column=1, padx=5)
