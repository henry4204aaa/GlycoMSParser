...
def open_prepare_dataset_window():
    ...
    def try_merge_selected_sample():
        ...
        try:
            ...
            pre_df, iondfindex, ion_df = mspval.directassign_files(files["excel"], files["csv"], debug = False)
    ...
    def write_method_file(exp_name, auto=False):
        ...
        for sname, files in experiment_projects[exp_name]["samples"].items():
            ...
            entry = {
                "csv": files["csv"],  #os.path.basename(files["csv"]),
                "excel": files["excel"], #os.path.basename(files["excel"]),
                "metadata": files["json"], #os.path.basename(files["json"]),
