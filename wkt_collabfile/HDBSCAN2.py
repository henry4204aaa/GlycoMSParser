...
for structure_label in pca_df['Structure'].unique():
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='markers',
        marker=dict(size=4, color=structure_data['Cluster'].apply(lambda x: colors[x % len(colors)])),
...
# Perform cross-validation and output classification report
...
classification_report_str = classification_report(y, y_pred, target_names=label_encoder.classes_)
