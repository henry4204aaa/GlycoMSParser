...
# 保留原始索引 (從1開始編號的原始索引)
data['OriginalIndex'] = data.index + 1  # +1 是為了確保索引從 1 開始，與 CSV 中的行數對應
...
pca_df['OriginalIndex'] = data['OriginalIndex']  # 保留原始索引
pca_df['Structure'] = data['Structure']  # 使用 'Structure' 列來進行上色

# 使用 Plotly 繪製 3D 散點圖，懸停顯示原始索引，點的大小設置較小
fig = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3', color='Structure', hover_data=['OriginalIndex'], size_max=5)
